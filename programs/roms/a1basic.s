...
rdkey:  LDA     IOSTATUS        ; Read status register
        BPL     rdkey           ; Loop if no key pressed (bit 7 = ready)
        LDA     IOREAD          ; Read key data (high bit set by hardware)
        RTS                     ; and return
...
Le3d5:  BIT     IOWRITE      ; (no-op read, display always ready)
        BMI     Le3d5        ; (never loops — display always ready)
        STA     IOWRITE      ; Write display data
        RTS                  ; and return
