...
440 P=10:P0=P:S9=200:S=0:B9=2:K9=0:X$="":V$=" IS "
...
820 FORI=1TO8:FORJ=1TO8:K3=0:Z(I,J)=0:R1=RND(0)
850 IFR1>.98THENK3=3:K9=K9+3:GOTO980
860 IFR1>.95THENK3=2:K9=K9+2:GOTO980
870 IFR1>.80THENK3=1:K9=K9+1
980 B3=0:IFRND(0)>.96THENB3=1:B9=B9+1
1040 G(I,J)=K3*100+B3*10+FNR(1):NEXTJ:NEXTI:IFK9>T9THENT9=K9+1
1100 IFB9<>0THEN1200
1150 IFG(Q1,Q2)<200THENG(Q1,Q2)=G(Q1,Q2)+120:K9=K9+1
1160 B9=1:G(Q1,Q2)=G(Q1,Q2)+10:Q1=FNR(1):Q2=FNR(1)
1200 K7=K9:IFB9<>1THENX$="S":V$=" ARE "
1230 PRINT"YOUR ORDERS ARE AS FOLLOWS:"
1240 PRINT"     DESTROY THE";K9;" KLINGON WARSHIPS WHICH HAVE INVADED"
1252 PRINT"   THE GALAXY BEFORE THEY ATTACK FEDERATION HQ"
1260 PRINT"   ON STARDATE";T0+T9;" THIS GIVES YOU";T9;" DAYS. THERE";V$
1272 PRINT" ";B9;" STARBASE";X$;" IN THE GALAXY FOR RESUPPLY"
1280 PRINT:REM READY PROMPT REMOVED
1300 I=RND(0):REM IF INP(1)=13 THEN 1300
...
1780 GOSUB8670:K(I,1)=R1:K(I,2)=R2:K(I,3)=S9*(0.5+RND(0)):NEXTI
1820 IFB3<1THEN1910
...
2020 PRINT:PRINT"** FATAL ERROR ** YOU'VE STRANDED YOUR SHIP IN "
2030 PRINT"SPACE":PRINT"YOU HAVE INSUFFICIENT MANEUVERING ENERGY,";
2040 PRINT" AND SHIELD CONTROL":PRINT"IS INCAPABLE OF CROSS";
2050 PRINT"-CIRCUITING TO ENGINE ROOM!!":GOTO6220
2060 INPUT"COMMAND";A$
2061 A$=LEFT$(A$,3)
2062 IFA$="NAV"THEN2300
2064 IFA$="SRS"THEN1980
2066 IFA$="LRS"THEN4000
2068 IFA$="PHA"THEN4260
2070 IFA$="TOR"THEN4700
2072 IFA$="SHE"THEN5530
2074 IFA$="DAM"THEN5690
2076 IFA$="COM"THEN7290
2078 IFA$="XXX"THEN6270
...
2470 PRINT"WARP ENGINES DAMAGED. MAX SPEED = WARP 0.2":GOTO1990
...
3165 Z7=0
3170 FORI=1TON:S1=S1+X1:S2=S2+X2
3175 IFS1<1ORS1>=9ORS2<1ORS2>=9THENZ7=1:I=N:GOTO3360
3240 S8=INT(S1)*24+INT(S2)*3-26:IFMID$(Q$,S8,2)="  "THEN3360
3320 S1=INT(S1-X1):S2=INT(S2-X2):PRINT"WARP ENGINES SHUT DOWN AT ";
3350 PRINT"SECTOR";S1;",";S2;" DUE TO BAD NAVAGATION":I=N:GOTO3360
3360 NEXTI:IFZ7=1THEN3500
3362 S1=INT(S1):S2=INT(S2)
...
3500 X=8*Q1+X+N*X1:Y=8*Q2+Y+N*X2:Q1=INT(X/8):Q2=INT(Y/8)
3505 S1=INT(X-Q1*8)
...
4265 IFK3>0THEN4330
4270 PRINT"SPOCK REPORTS 'SENSORS SHOW NO ENEMY SHIPS"
...
4410 E=E-X:IFD(7)<0THENX=X*RND(0)
4450 H1=INT(X/K3):FORI=1TO3:IFK(I,3)<=0THEN4670
4480 H=INT((H1/FND(0))*(RND(0)+2)):IFH>.15*K(I,3)THEN4530
4500 PRINT"SENSORS SHOW NO DAMAGE AT";K(I,1);",";K(I,2):GOTO4670
4530 K(I,3)=K(I,3)-H:PRINTH;" UNIT HIT ON KLINGON AT";K(I,1);",";
...
5150 I=3:FORJ9=1TO3:IFX3=K(J9,1)ANDY3=K(J9,2)THENI=J9
5180 NEXTJ9
5190 K(I,3)=0:GOTO5430
5210 A$=" * ":Z1=X:Z2=Y:GOSUB8830:IFZ3=0THEN5280
5260 PRINT"STAR AT";X3;",";Y3;" ABSORBED TORPEDO ENERGY.":GOSUB6000
5265 GOTO1990
...
5560 PRINT"ENERGY AVAILABLE =";E+S;:INPUT" UNITS TO SHIELDS";X
5580 IFX<0ORS=XTHENPRINT"<SHIELDS UNCHANGED>":GOTO1990
5590 IFX<=E+STHEN5630
5600 PRINT"SHIELD CONTROL REPORTS 'NOT THE FEDERATION TREASURY.'"
...
6120 IFRND(0)>.6ORH/S<=.02THEN6200
...
6490 IFD0=0THENA$=">!<":Z1=I:Z2=J:GOSUB8830:IFZ3=1THEND0=1
6540 NEXTJ:NEXTI
6550 IFD0=0THEN6650
6580 C$="DOCKED":E=E0:P=P0
...
7960 PRINT"MISSION MUST COMPLETE IN";.1*INT((T0+T9-T)*10);" STARDATES"
7970 X$="S":IFB9<2THENX$="":IFB9<1THEN8010
7980 PRINT"THE FEDERATION MAINTAINS";B9;" STARBASE";X$;" IN THE GALAXY"
7990 GOTO5690
...
8480 NEXTI:GOTO1990
8500 IFB3<>0THENPRINT"ENTERPRISE TO STARBASE:":W1=B4:X=B5:GOTO8120
